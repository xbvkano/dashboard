...
export default function NewConversationModal({
  ...
  const isEmployee = inboxKind === 'employee'
  const nameTrim = name.trim()
  const notesEnabled = !isEmployee && Boolean(nameTrim)
  ...
  const submit = async () => {
    ...
    if (!isEmployee && notes.trim() && !nameTrim) {
    ...
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center sm:p-4 bg-black/40">
      ...
      >
        <h2 id="new-conv-title" className="text-lg font-semibold text-slate-900">
          {isEmployee ? 'New employee conversation' : 'New conversation'}
        </h2>
        <p className="text-sm text-slate-600 mt-1">
          {isEmployee
            ? 'Text an employee from the admin Twilio line. Optional name is shown in the thread list only.'
            : 'Enter a phone number. Add a name to create a client and link this contact. Notes are only available when a name is set.'}
        </p>
        <div className="mt-4 space-y-3">
          ...
          <label className="block">
            <span className="text-sm font-medium text-slate-700">
              {isEmployee ? 'Name (optional)' : 'Name (optional)'}
            </span>
            <input
              type="text"
              value={name}
              ...
              className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-[15px] focus:outline-none focus:ring-2 focus:ring-blue-400/50"
              placeholder={isEmployee ? 'Employee name' : 'Client name'}
              autoComplete="name"
            />
          </label>
          {!isEmployee && (
